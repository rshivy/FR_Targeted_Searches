...
# This is the version from the 15yr stochastic analysis notebooks
class EmpiricalDistribution2D_v2(object):
...
class EmpiricalDistribution1D_v2(object):
    def __init__(self, ed2d):
        self.ndim = ed2d['ndim']
        self.param_name = ed2d['param_name']
        self._Nbins = np.array(ed2d['_Nbins'])
        self._edges = np.array(ed2d['_edges'])
        self._wids = np.array(ed2d['_wids'])
        self._pdf = np.array(ed2d['_pdf'])
...
def make_empirical_distributions_from_core(c, paramlist,
                                           burn=0, nbins=81,
                                           filename='distr.pkl',
    ...
    for pl in paramlist:
        ...
        if len(pl) == 1:
            ...
            prior_min = float(c.priors[idx][c.priors[idx].index('pmin')+
                                            5:c.priors[idx].index(', pmax')])
            prior_max = float(c.priors[idx][c.priors[idx].index('pmax')+
                                            5:c.priors[idx].index(')')])

            # get the bins for the histogram
            bins = np.linspace(prior_min, prior_max, nbins)

            new_distr = EmpiricalDistribution1D(pl[0], c.chain[c.burn:, idx],
                                                bins)
            ...
            new_distr = EmpiricalDistribution2D(pl, c.chain[burn:, idx].T,
                                                bins)

            distr.append(new_distr)

        else:
            msg = ('WARNING: only 1D and 2D empirical distributions '
    ...
    if save_dists:
        if len(distr) > 0:
            ...
            fn = filename
            msg = f'The empirical distributions have been pickled to {fn}.'
            logger.info(msg)
        else:
            msg = 'WARNING: No empirical distributions were made!'
            logger.warning(msg)
