...
from targeted_cws_ng15.empirical_distr_new import (EmpiricalDistribution2D,
                                                   EmpiricalDistribution1D,
                                                   EmpiricalDistribution2DKDE,
                                                   EmpiricalDistribution1DKDE,
                                                   EmpiricalDistribution1D_v2,
                                                   EmpiricalDistribution2D_v2)
...
def extend_emp_dists(pta, emp_dists, npoints=100_000, save_ext_dists=False, outdir='chains'):
    ...
    for emp_dist in emp_dists:
        if (isinstance(emp_dist, EmpiricalDistribution2D) or
            isinstance(emp_dist, EmpiricalDistribution2DKDE) or
            isinstance(emp_dist, EmpiricalDistribution2D_v2)):
            ...
            for ii, (param, nbins) in enumerate(zip(emp_dist.param_names, emp_dist._Nbins)):
                ...
                if isinstance(emp_dist, EmpiricalDistribution2D) or isinstance(emp_dist, EmpiricalDistribution2D_v2):
            ...
            new_emp_dists.append(new_emp)
              ...
              isinstance(emp_dist, EmpiricalDistribution1DKDE) or
              isinstance(emp_dist, EmpiricalDistribution1D_v2)):
            ...
            # skip if emp dist already covers the prior
            if isinstance(emp_dist, EmpiricalDistribution1D) or isinstance(emp_dist, EmpiricalDistribution1D_v2):
...
class JumpProposal(object):
    ...
    def draw_from_expdip_prior(self, x, iter, beta):
    
        q = x.copy()
        lqxy = 0
    
        dmexp_names = [sname for sname in self.snames if 'exp' in sname]
        signal_name = np.random.choice(dmexp_names)
        param = np.random.choice(self.snames[signal_name])
        if param.size:
            idx2 = np.random.randint(0, param.size)
            q[self.pmap[str(param)]][idx2] = param.sample()[idx2]
    ...
    def draw_from_gwb_log_uniform_distribution(self, x, iter, beta):
        ...
        signal_name = [par for par in self.pnames
                       if ('gw' in par and 'log10_A' in par) or
                       ('crn' in par and 'log10_A' in par)][0]
    ...
    def draw_from_gw_rho_prior(self, x, iter, beta):
        ...
        pname = [pnm for pnm in parnames
                 if (('gw' in pnm or 'crn' in pnm) and 'rho' in pnm)][0]
